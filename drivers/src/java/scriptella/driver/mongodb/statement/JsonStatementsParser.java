package scriptella.driver.mongodb.statement;
...
import scriptella.driver.mongodb.operation.MongoOperation;
...
/**
 * Represents a parser for Mongodb statements in JSON format . Parsing and
 * pre-compilation is done at construction time. After construction only
 * parameters are substituted using
 * {@link #setParameters(scriptella.spi.ParametersCallback)} methods. This
 * approach has a significant performance benefit over parsing JSON on each
 * invocation.
 * <h3>Parameters syntax</h3>
 * <code><b>"?</b>variable<b>"<b></code> or <code><b>"?</b>{variable or JEXL
 * expression}<b>"</b></code> syntax is supported. Please note that
 * variable/expressions bindings <em>must be enclosed in single or double
 * quotes</em>. This decision was made to keep parameterized JSON compatible
 * with plain JSON, so that standard parsers can read it.
 * <p/>
 * <p>
 * <b>Note: </b>This class is not thread-safe</p>
 *
...
public class JsonStatementsParser {
    ...
    static class ObjectBinding {
        ...
        /**
         * Name of the property of the JSON object to which the binding is
         * assigned
         */
        String property;
        /**
